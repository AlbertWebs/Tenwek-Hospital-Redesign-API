...
<head>
    ...
    <title>@yield('title', $site['site_name'] ?? 'Tenwek Hospital')@if(! empty(trim($site['layout_title_suffix'] ?? ''))) — {{ $site['layout_title_suffix'] }}@endif</title>
    <meta name="description" content="@yield('meta_description', $site['meta_description_default'] ?? '')">
    @if(! empty(trim($site['meta_keywords'] ?? '')))
...
<body id="top" class="min-h-screen flex flex-col bg-slate-50 text-slate-800 font-sans antialiased">
    ...
    <div class="bg-teal-700 text-white text-center py-2 px-4 text-sm">
        <span>{{ $site['top_bar_message'] ?? 'To book our ambulance, contact the Tenwek Hospital Coverage team on' }}</span>
        <a href="{{ $site['ambulance_tel_href'] ?? '#' }}" class="font-semibold hover:underline ml-1 whitespace-nowrap">{{ $site['ambulance_phone'] ?? '0727 033 725' }}</a>.
    </div>
            ...
            <div class="flex items-center h-14 lg:h-12">
                <div class="flex-1 flex items-center lg:min-w-0">
                    <a href="{{ route('home') }}" class="lg:hidden flex items-center gap-2 shrink-0">
                        <span class="text-lg font-semibold text-teal-700">{{ $site['site_name'] ?? 'Tenwek Hospital' }}</span>
                    </a>
                </div>
    ...
    <footer class="bg-slate-800 text-slate-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="mt-10 pt-8 border-t border-slate-700 flex flex-wrap items-center justify-center gap-x-8 gap-y-2 text-sm">
                <span>Customer service: <a href="{{ $site['primary_tel_href'] ?? '#' }}" class="font-semibold text-white hover:underline">{{ $site['phone_primary'] ?? '0700 499 699' }}</a></span>
                <span>Ambulance / Coverage: <a href="{{ $site['ambulance_tel_href'] ?? '#' }}" class="font-semibold text-white hover:underline">{{ $site['ambulance_phone'] ?? '0727 033 725' }}</a></span>
            </div>
            <div class="mt-6 pt-6 border-t border-slate-700 text-center text-sm text-slate-500">
                &copy; {{ date('Y') }} {{ $site['site_name'] ?? 'Tenwek Hospital' }}. All rights reserved.
            </div>
        </div>
    </footer>
